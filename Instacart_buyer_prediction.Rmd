...
breakdt1<-habit1.order2[user_id %in% c(1:100000)][, .(Aggregate.count = .N), by=.(user_id, product_id)][order(user_id,-Aggregate.count)]
breakdt2<-habit1.order2[user_id %in% c(100001:200000)][, .(Aggregate.count = .N), by=.(user_id, product_id)][order(user_id, -Aggregate.count)]
breakdt3<-habit1.order2[user_id %in% c(200001:length(numberuserids))][, .(Aggregate.count = .N), by=.(user_id, product_id)][order(user_id, -Aggregate.count)]
